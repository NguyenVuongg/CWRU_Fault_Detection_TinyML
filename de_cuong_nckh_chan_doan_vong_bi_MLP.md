# TỔNG QUAN DỰ ÁN NGHIÊN CỨU KHOA HỌC (BẢN CHỐT CUỐI CÙNG — RÚT GỌN, CHỈ GIỮ MLP)
...
> **Ghi chú cập nhật:** Phiên bản này đã bỏ toàn bộ phần đối chứng CNN1D nhằm làm nhẹ phạm vi đồ án — đề tài chỉ còn tập trung vào MLP trên đặc trưng chọn lọc theo kiến thức miền. RQ2/H2 (MLP vs. CNN1D) đã được loại bỏ; các RQ còn lại được đánh số lại thành RQ1–RQ4 (thay vì RQ1–RQ5).
...
3. Kiểm chứng thực nghiệm khả năng tổng quát hóa của MLP (trên đặc trưng chọn lọc theo kiến thức miền) qua kiểm định LOLO — làm cơ sở đánh giá tính khả thi khi triển khai lên MCU tài nguyên thấp.

**Phạm vi & giới hạn:**
- Chỉ sử dụng kiến trúc MLP trên đặc trưng chọn lọc theo kiến thức miền — không đối chứng với các kiến trúc deep learning học trực tiếp từ tín hiệu thô (CNN1D hay tương tự), nhằm giữ phạm vi đồ án gọn nhẹ và tập trung nguồn lực vào phần tối ưu tài nguyên nhúng.
...
Từ các quan sát ở 0.1–0.3, đề tài hình thành 4 câu hỏi nghiên cứu (RQ1–RQ4) và giả thuyết tương ứng, trình bày chi tiết ở phần **RESEARCH QUESTIONS & HYPOTHESES** ngay sau đây — đây là cầu nối chuyển từ giai đoạn khảo sát sang giai đoạn thiết kế/thực thi pipeline chính thức (Giai đoạn 1–4).
...
### RQ2 — Đánh đổi giữa Feature Selection và tài nguyên/độ chính xác

**RQ2:** Việc rút gọn bể đặc trưng đầy đủ xuống ~30 đặc trưng (qua Random Forest Importance) ảnh hưởng thế nào đến độ chính xác phân loại và chi phí tài nguyên (RAM/MACC) trên MCU?

**H2:** Giảm số chiều đặc trưng sẽ làm giảm đáng kể RAM/MACC, trong khi độ chính xác giảm không đáng kể — thậm chí có thể tăng nhẹ nếu loại bỏ được đặc trưng nhiễu góp phần overfitting.
...
| RQ2 | Bể đặc trưng đầy đủ vs. ~30 đặc trưng | Accuracy, RAM, MACC, Flash |
| RQ3 | Square-Law vs. Hilbert Transform | Latency, RAM |
| RQ4 | Goertzel vs. FFT toàn dải | MACC, RAM |
...
Khuyến nghị chạy Random Forest baseline trước tiên (nhanh, ít nhạy tham số) để xác nhận toàn bộ pipeline Giai đoạn 1 hoạt động đúng, trước khi chuyển sang MLP.
...
Mục tiêu là kiểm chứng khả năng tổng quát hóa qua LOLO của MLP dựa trên đặc trưng miền kỹ thuật, làm cơ sở đánh giá tính khả thi khi triển khai TinyML lên STM32. Chi phí tài nguyên (RAM/Flash/MACC) của mô hình được đo và báo cáo ở Giai đoạn 3.
...
- **Bảng 2**: Bể đặc trưng đầy đủ (Full) vs. ~30 đặc trưng (sau Feature Selection) — RAM/MACC giảm bao nhiêu %, F1-Score thay đổi thế nào. *(RQ2/H2)*
- **Bảng 3a**: Square-Law Demodulation vs. Hilbert Transform — thời gian thực thi (ms) trên ARM Cortex-M. *(RQ3/H3)*
- **Bảng 3b**: Goertzel (M tần số đã chốt) vs. FFT toàn dải — RAM và MACC. *(RQ4/H4)*
